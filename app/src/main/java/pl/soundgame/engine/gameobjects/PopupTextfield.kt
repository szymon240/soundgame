...
class PopupTextfield(background: Bitmap, popupText: String = "", popupAnswer1: String = "", popupAnswer2: String = "", val duration: Int = -1, id: String = "", val context: MainActivity) : GameObject(bitmap = background , id= id) {
    ...
    private var textFieldOn = false
    private var afterInputCallback: (String) -> Unit
    private var nextUpdate = false
    ...
    init {
        ...
        var text = ""
        if (popupText == "") {
            text = context.getString(R.string.username_query)
        } else {
            text = popupText
        }

        popupTextBox = TextBox(id = "${id} - Textbox", size = 32f, width = 300, initialText = text)
        popupTextBox.setOriginPosition(y = 0.2f)
        popupTextBox.scale(0.5f)

        answerButton1 = TextBox(
            id = "${id} - Answer1", size = 24f, initialText = popupAnswer1,
            color = Color.GREEN, height = 100, width = 300
        )
        answerButton1.setOriginPosition(y = -0.15f)
        answerButton1.scale(0.25f)

        answerButton2 = TextBox(
            id = "${id} - Answer2", size = 24f, initialText = popupAnswer2,
            color = Color.RED, height = 100, width = 300
        )
        answerButton2.setOriginPosition(y = -0.3f)
        answerButton2.scale(0.25f)

        textField = Button(loadTextureBitmap("textField.png", context), id = "$id - TextField")
        textField.setOriginPosition(y = 0.025f)
        textField.scale(0.1f)
        afterInputCallback = { enteredText ->
            textFieldOn = false
            Log.i("PopupTextfield", "Entered text: $enteredText")
            val bitmapWithText = createTextTexture(
                enteredText,
                background = loadTextureBitmap("textField.png", context),
                color = Color.BLACK,
                size = 92f
            )
            textField.changeBaseBitmap(bitmapWithText)
            inputText = enteredText
        }

        textField.onClickAction {
            val mainActivity = context as? MainActivity
            textFieldOn = true
            mainActivity?.let {
                it.showKeyboard { enteredText ->
                    inputText = enteredText
                    nextUpdate = true
                    context.hideKeyboard()
                }
            } ?: run {
                Log.e("PopupTextfield", "Context is not MainActivity")
            }
        }
    }
